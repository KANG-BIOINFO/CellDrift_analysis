...
def extract_from_prediction(df_deg, DE_factor, category, p_fdr_thred = 0.05):
    df_sub = df_deg.loc[df_deg["DE_FacLoc"] == DE_factor, : ].copy()
...
if __name__ == "__main__":
    ...
    for threshold in [25,50,75]:
        raw_path = "/data/aronow/Kang/CellDrift/test/test_benchmark/simulated_ifn_data/test_deProb_h5ad_v1/"
        
        df_output = pd.DataFrame(columns = ["TPR", "FDR", "Replicates", "DEFacLoc", "Method"])
        ...
        for rep_id in tqdm(np.arange(1,11)):
            ...
            for ct_file in ct_files:
                ...
                DE_facloc = float(ct_file.split("sim_deProb_")[1].split(".h5ad")[0])
                
                genes_up, genes_down, genes_neg = define_golden(adata, thred = threshold)

                # ct pairwise
                pred_up, pred_down, pred_neg = extract_from_prediction(df_pairwise, DE_facloc, "pairwise")
                TPR_score = TPR(genes_up, genes_down, pred_up, pred_down)
                FDR_score = FDR(genes_up, genes_down, genes_neg, pred_up, pred_down)
                
                df_output.loc[row_id, :] = [TPR_score, FDR_score, rep_name, DE_facloc, "CellDrift"]
                row_id += 1

                # others
                for name, df_deg in zip(test_names, df_others):
                    pred_up, pred_down, pred_neg = extract_from_prediction(df_deg, DE_facloc, "others")
                    TPR_score = TPR(genes_up, genes_down, pred_up, pred_down)
                    FDR_score = FDR(genes_up, genes_down, genes_neg, pred_up, pred_down)

                    df_output.loc[row_id, :] = [TPR_score, FDR_score, rep_name, DE_facloc, name]
                    row_id += 1
                
                # mast
                pred_up, pred_down, pred_neg = extract_from_prediction(df_mast, DE_facloc, "MAST")
                TPR_score = TPR(genes_up, genes_down, pred_up, pred_down)
                FDR_score = FDR(genes_up, genes_down, genes_neg, pred_up, pred_down)
                df_output.loc[row_id, :] = [TPR_score, FDR_score, rep_name, DE_facloc, "MAST"]
                row_id += 1

        df_output.to_csv("BenchDEfactor_metrics_threshold_" + str(threshold) + ".txt", sep = "\t")
